...
def get_arguments():
    ...
    # REQUIRED arguments:
    parser.add_argument(
        "-q", "--query", help="Query file in a .faa format", required=True, type=str)
    parser.add_argument("-o", "--output_prefix",
                        help="Path and prefix of the output files", required=True, type=str)
    # OPTIONAL arguments:
    parser.add_argument("-d", "--database", help="Path to the (processed) kegg database", required=False,
                        default='/nfs/nas22/fs2202/biol_micro_sunagawa/Projects/PAN/GENERAL_METAGT_ANALYSIS_PAN/data/resources/soft/kegg_annotation/kegg_db/kegg/kegg_for_prokka-with_ko.dmnd', type=str)
    parser.add_argument("-k", "--ko_dictionaries", help="Path to the ko directory containing the mapping files.",
                        required=False, default='/nfs/nas22/fs2202/biol_micro_sunagawa/Projects/PAN/GENERAL_METAGT_ANALYSIS_PAN/data/resources/soft/kegg_annotation/kegg_db/kegg/ko', type=str)
    parser.add_argument("-t", "--threads", help="Number of threads to use.",
                        required=False, default=8, type=int)
    parser.add_argument("-m", "--mode", help="Specify if diamond should run in default, 'sensitive' or 'more-sensitive' mode",
                        required=False, default="more-sensitive", type=str)
...
def check_diamond_db(args):
    ...
    #print('Diamond executable {}'.format(shutil.which('diamond'))
    if args.database.endswith(".dmnd"):
        print("The database {} ends with '.dmnd' so I assume it's all good to go.".format(args.database))
        db_name = args.database
    else:
        db_prefix = "".join(args.database.split(".")[0:-1])
        db_name = db_prefix + '.dmnd'
        command = "diamond makedb --in {} --db {}".format(
            args.database, db_prefix)
        returncode = subprocess.check_call(command, shell = True)
        if returncode != 0:
            raise Exception("Diamond commandline {} finished with returncode {}...".format(
                command, returncode))
        if not Path(db_name).exists():
            raise FileNotFoundError(
                "The diamond database file {} doesn't exist... something went wrong.".format(db_name))
    return db_name
...
def run_diamond(args):
    ...
    command = "diamond blastp -q {} -d {} -o {} --threads {} {} &> {}.log".format(
        args.query, args.database, temp_output, args.threads, fixed_params, args.output_prefix)
    returncode = subprocess.check_call(command, shell = True)
    ...
    if not Path(temp_output).exists():
        raise FileNotFoundError(
            "The diamond output file {} doesn't exist... something went wrong.".format(temp_output))
    return temp_output
...
def process_diamond_output(diamond_temp, diamond_processed):
    ...
    """
    print("Processing the diamond output")
    # Use the filtered out for debugging
    with open(diamond_temp) as handle, open(diamond_processed, "w") as target:
        ...
        header = ["qseqid", "sseqid", "qlen", "slen", "length", "pident",
                  "qcovhsp", "evalue", "bitscore", "max_bitscore", "norm_bitscore"]
        target.write("\t".join(header) + "\n")
        for line in handle:
            line = line.rstrip().split("\t")
            seq = line[-1]
            line[-1] = get_max_bitscore(seq)
            norm_bitscore = round(float(line[-2]) / float(line[-1]), 3)
...
def get_kegg_annotations(args, processed_diamond_output):
    ...
    for gene in set(diamond_df.index):
        ...
        if isinstance(gene_df, pandas.DataFrame):
            gene_df = gene_df[gene_df.norm_bitscore >=
                              (max(gene_df.norm_bitscore) - 0.1)]
            # We want to remove empty strings (from None) that make things ugly
            KOs = ','.join([i for i in set(gene_df.KO) if i != ""])
            COGs = ','.join([i for i in set(gene_df.COG) if i != ""])
            ECs = ','.join([i for i in set(gene_df.EC) if i != ""])
            Descriptions = ' / '.join(
                [i for i in set(gene_df.Description) if i != ""])
    ...
    annotation_df = pandas.DataFrame.from_dict(
        annotation_dict, orient="index", columns=["KO", "COG", "EC", "Description"])
